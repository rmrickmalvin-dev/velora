...
| CODAL-DEC-305 | ACEITA | PASSO 45 treats the existing 18 Client Components as a reviewed performance budget instead of moving validated Server Component route entries to the client | Prevent silent client-runtime growth while preserving required interactive islands |
| CODAL-DEC-306 | ACEITA | The current conceptual Product visual layer remains SVG-only and lightweight; `next/image` is not introduced when no raw or raster image-rendering surface exists | Avoid optimization machinery that does not reduce the measured current payload |
| CODAL-DEC-307 | ACEITA | Home, Category and Product remain Server Component entry routes while interactive behavior stays inside existing Client Component boundaries | Preserve static generation and minimize public route JavaScript responsibility |
| CODAL-DEC-308 | ACEITA | Public Home, Category and Product metadata reuse the SEO model for Open Graph and Twitter summary records while preserving canonical and language alternates | Strengthen share and crawler metadata without duplicating public copy logic |
| CODAL-DEC-309 | ACEITA | Robots discovery excludes Account, Admin, Checkout, Login and Orders demo journeys, while sitemap discovery contains only public Home, Category and Product journeys | Keep portfolio commerce utility routes out of public search discovery |
...
| CODAL-DEC-324 | ACEITA | PASSO 47 prepares a provider-neutral local Release Candidate because the diagnostic found no Git remote, upstream branch, deployment provider or real public origin | Avoid inventing external release identity or infrastructure |
| CODAL-DEC-325 | ACEITA | Repository text files use an explicit `.gitattributes` policy of `* text=auto eol=lf` without mass-renormalizing historical files in PASSO 47 | Make line-ending policy repository-owned instead of dependent on machine-level `core.autocrlf` |
| CODAL-DEC-326 | ACEITA | Release readiness is exposed through `release:preflight`, `release:candidate` and strict `release:deploy-check` commands implemented with a provider-neutral Node script | Keep local RC validation reproducible while separating external deployment requirements |
| CODAL-DEC-327 | ACEITA | `NEXT_PUBLIC_SITE_URL` remains blank in `.env.example`; strict deploy-check requires a real non-placeholder HTTP or HTTPS origin | Preserve the no-fake-production-host rule through BUILD 06 |
